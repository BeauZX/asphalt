...
Runs on a **Raspberry Pi 5** with two IMX219 CSI cameras attached. Target use is
in-vehicle road surveying.
...
**No GPU.** `torch.cuda.is_available()` is always False. Everything is CPU
inference on 4 cores.

**Grid cell count, not resolution, is the bottleneck.** Measured with NCNN and
PyTorch alike: 4×10=40 cells takes ~1.2 s per pass, 3×5=15 cells ~0.6 s. Dropping
`imgsz` from 256 to 128 barely helps at 40 cells (per-cell fixed overhead dominates)
but does help once the cell count is low. NCNN export was tried and only gave
~1.3× — not worth the extra dependency, so the plain `.pt` model is used.

**Live mode must not analyse synchronously.** At ~1.4 s per analysis, a
read→analyse→display loop produces a video that updates once per second and a
preview that is unusable. `GridAnalyzer` runs inference on a background thread
while the main loop displays and records at full camera rate, filling in with the
most recent grid result. `submit()` deliberately keeps only the newest frame and
drops the rest — queuing would make the displayed colours drift further and further
behind reality.
...
- `src/grid.py` — `classify_grid()`, `draw_grid_overlay()`, and `GridTracker`,
  ...
  calls it per analysis (~1.4 s). Same logic, different cadence.
...
`camera.torch_threads` caps YOLO's core usage so recording and encoding keep
their share; raising it makes the preview stutter.

## Measured performance

Live mode: ~24.7 fps display/record, grid colours refresh ~1.4 s, no thermal
...
Video mode on the sample footage (`asphalt.MP4`, 2704×1520 @ 239.76 fps, 3635
frames) takes roughly 84 minutes. Known unfixed issues, listed in README.md: no
frame skipping, output inherits the 240 fps source rate (so it appears frozen on a
60 Hz display), and files are large because of the `mp4v` codec.
